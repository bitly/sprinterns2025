...
func CreateEvent(event models.Event) (*models.Event, error) {
	_, err := dbmap.Query(
		"INSERT INTO event (title, date, time, location, host_name, description, contact_info, public_private, num_of_RSVP, max_attendees, image_url, typeOfevent) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
	...
	eventrow, err := dbmap.Query(
		"SELECT event_id, title, date, time, location, host_name, description, contact_info, public_private, num_of_RSVP, max_attendees, image_url, typeOfevent FROM event ORDER BY event_id DESC LIMIT 1")
...
func GetEvent(eventID int) (*models.Event, error) {
	var events []models.Event
	eventrow, err := dbmap.Query(
		"SELECT event_id, title, date, time, location, host_name, description, contact_info, public_private, num_of_RSVP, max_attendees, image_url, typeOfevent FROM event WHERE event_id=?;",
		eventID)
	if err != nil {
		return nil, err
	}
...
func GetAllPublicEvents() ([]models.Event, error) {
	...
	rows, err := dbmap.Query(
		"SELECT  event_id, title, date, time, location, host_name, description, contact_info, public_private, num_of_RSVP, max_attendees, image_url, typeOfevent FROM event WHERE public_private = 'public'")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func UpdateEventByEventId(eventID int, updatedEvent models.Event) (*models.Event, error) {
	_, err := dbmap.Exec(
		"UPDATE event SET title = ?, date = ?, time = ?, location = ?, host_name = ?, description = ?, contact_info = ?, public_private = ?, num_of_RSVP = ?, max_attendees = ?, image_url = ?, typeOfevent = ? WHERE event_id = ?",
...
func GetEventsByField(field, value string) ([]models.Event, error) {
	...
	query := "SELECT event_id, num_of_RSVP, title, date, time, location, host_name, description, contact_info, public_private, max_attendees, image_url, typeOfevent FROM event WHERE " + field + "=?"
